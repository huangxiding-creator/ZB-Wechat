...
1. **扫描监控** — 定时扫描518个工程总承包相关公众号的最新文章（智能缓存+限流保护，31个分类全覆盖）
...
| 启动自检 | 运行前自动验证API密钥有效性，session过期立即报错 |
| 发布自动重试 | 企业微信/邮件推送失败时每15分钟自动重试，最多5次 |
...
│   │   ├── validate-accounts.ts # 公众号列表API验证工具（支持断点续传）
...
每篇干货文章从四个维度进行严格评分（1-5分），AI以严厉评委标准打分：

| 维度 | 1分 | 3分 | 5分 |
|------|-----|-----|-----|
| 技术深度 | 无技术细节 | 有具体方法 | 可量化参数/公式/规范条款 |
| 实操价值 | 纯理论 | 有一定参考 | 可直接照搬的模板/流程/判例结论 |
| 新颖度 | 行业常识（大部分文章） | 有新视角 | 百里挑一的独创性观点 |
| EPC相关度 | 间接相关 | 较相关 | 直接针对EPC核心业务 |

优先级判定（宁缺毋滥）：
- **🔥必读**: 总分≥17 且 至少一个维度=5分
- **⭐推荐**: 总分≥14
- **📌参考**: 总分≥8

精选机制：每期情报从所有干货中精选不超过10篇（必读全保留+推荐按分数补位），确保读者15-20分钟即可消化精华。
...
系统监控518个公众号，按31个分类全覆盖EPC行业生态：

| 分类 | 数量 | 分类 | 数量 |
|------|------|------|------|
| 施工总承包 / 工程总承包 | 19 | 综合建筑央企 | 10 |
| 工程法律 / 合同 / 索赔 | 33 | 中建系工程局 | 13 |
| 工程审计 / 造价 | 17 | 中铁系工程局 | 12 |
| 工程咨询 / 全过程咨询 | 27 | 中铁建系 | 2 |
| 项目管理 | 11 | 地方建工 / 施工企业 | 17 |
| 施工技术 / 建筑科技 | 9 | 电力 / 能源设计院 | 36 |
| 行业洞察 / 海外工程 | 11 | 化工 / 石化设计院 | 14 |
| 标准规范 / 政策研究 | 3 | 冶金 / 钢铁设计院 | 15 |
| 招投标 / 采购 | 3 | 铁路 / 交通设计院 | 31 |
| BIM / 数字化 | 1 | 建筑设计院 | 62 |
| 综合前沿 | 4 | 水利 / 市政设计院 | 10 |
| | | 煤炭 / 矿业设计院 | 7 |
| | | 军工 / 核工业 | 4 |
| | | 装饰 / 其他 | 2 |
| 国家级行业协会 | 17 | 省级造价管理协会 | 25 |
| 省级建筑业协会 | 27 | 省级监理协会 | 28 |
| 省级勘察设计协会 | 26 | 省级招投标协会 | 22 |

可通过编辑 `公众号监控列表.txt` 自行增减，运行 `npx ts-node src/intelligence/validate-accounts.ts` 验证有效性。
