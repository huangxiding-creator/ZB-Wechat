...
4. **情报快报** — 生成"总包公号情报"Markdown + 精美PDF，推送企业微信 + 邮件发送
5. **可视化管理** — Web UI界面，所有参数在线配置，实时查看扫描进度
6. **PDF推送** — 自动生成精美PDF情报并通过邮箱发送，方便存档和转发
...
每次运行自动生成精美PDF情报（深色渐变标题、卡片式文章布局），通过邮箱发送附件。PDF使用Playwright渲染，支持中文字体和精美排版。
